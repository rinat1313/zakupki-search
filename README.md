...
У каждой настройки уже есть уникальный стабильный UUID — поле **`id`** в `search_profiles` / API.

| Где | Имя поля | Смысл |
|-----|----------|--------|
| `zakupki-search` | `id` / `profile_id` | PK конфигурации поиска |
| `zakupki-core` | `search_profile_id` | ссылка на эту настройку у тендера |

- Выдаётся при создании профиля, **не меняется** при update имени/фильтров.
- Не использовать `name` как ключ — имя уникально только в рамках пользователя и может меняться.
- Между БД сервисов физического FK нет: core хранит UUID как логическую ссылку.
- Схема актуальна: отдельный «второй» идентификатор не нужен.
